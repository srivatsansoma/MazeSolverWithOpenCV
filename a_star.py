...
def a_star(start, end, connections, weights, r_size):
    ...
    for i in range(len(connections)):
        del_y = round((i - start) % r_size)
        del_x = abs(del_y * r_size - (i - start))
        h_weights.append(del_x + del_y)
    ...
    while run_while:
        for i in range(len(connections[next_node])):
            ...
            if not visited[connections[next_node][i]] and new_dist < shortest_dist_and_prev_node[connections[next_node][i]][0]:
